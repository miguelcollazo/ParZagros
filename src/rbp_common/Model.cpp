...
#ifdef _OPENMP
  // Local vectors for each threads
...
#pragma omp parallel num_threads(4) firstprivate(nb_fg_l, nb_bg_l)
{
    ...
    // Reduce private arrays from every thread
    #pragma omp critical
    {
...
#ifndef _OPENMP
  // Run this code only in "serial" mode, without optimizations
  for (size_t i = 0; i < site_indic.size(); ++i) {
    for (size_t j = 0; j < site_indic[i].size(); ++j) {
      for (size_t k = 0; k < motif_width; ++k) {
        // can't count anything for N's
...
calculate_number_of_bases_fg_bg_str(const vector<string> &seqs,
                                    ...
                                    const double pseudoCount=1) {
  // TODO: Apply same optimization as calculate_number_of_bases_fg_bg
...
maximization_seq(const vector<string> &seqs,
  ...
  // Usually matrix size is small, not worth to parallelize it
  for (size_t i = 0; i < matrix.size(); ++i) {
...
Model::calculate_zoops_log_l(const vector<string> &sequences,
  ...
  double ret = 0.0;
  // Const small size of alphabet_size, not wortk to parallelize it
  for (size_t i = 0; i < alphabet_size; ++i) {
    ret += (nb_bg[i] * log(f[i]));
    if (!std::isfinite(ret)) {
      stringstream ss;
  ...
  //--------
  double ret_l = 0.0; // Not technically a local
...
Model::expectation_maximization_seq_de(const vector<string> &seqs,
  ...
  for (size_t i = 0; i < max_iterations; ++i) {  // max iterations is a constant (10UL)
    if (Model::DEBUG_LEVEL >= 1) {
